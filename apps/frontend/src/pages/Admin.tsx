...
const canManage = (current: User, target: User): boolean => {
  if (current.id === target.id) return false;
  if (current.role === 'SUPER_ADMIN') return true;
  if (current.role === 'ADMIN') return target.role === 'EMPLOYEE' || target.role === 'HR';
  if (current.role === 'HR') return target.role === 'EMPLOYEE';
  return false;
};
...
export const AdminDashboard: React.FC = () => {
  ...
  }, [user, fetchHistory, fetchUsers, fetchSchedules]);
  ...
  const roleFiltered = users.filter((u) => {
    ...
    if (user.role === 'ADMIN') return u.role === 'EMPLOYEE' || u.role === 'HR';
    if (user.role === 'HR') return u.role === 'EMPLOYEE';
    return false;
  });

  const filteredUsers = roleFiltered.filter(u => u.dni.includes(userSearch) || u.firstName.toLowerCase().includes(userSearch.toLowerCase()) || u.lastName.toLowerCase().includes(userSearch.toLowerCase()));
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col font-sans">
      ...
      <header className="bg-slate-900 border-b border-slate-800 px-4 md:px-8 py-4 flex justify-between items-center sticky top-0 z-10">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/')} className="text-sm font-medium text-slate-400 hover:text-white transition">Ir al Kiosko</button>
          <div className="w-px h-6 bg-slate-800" />
          <button onClick={() => { logout(); navigate('/'); }} className="flex items-center gap-2 text-sm font-medium text-rose-400 hover:text-rose-300 transition bg-rose-400/10 hover:bg-rose-400/20 px-3 py-1.5 rounded-lg"><LogOut size={16} /> Salir</button>
        </div>
      </header>

      {/* Tabs */}
      <div className="flex border-b border-slate-800 px-4 md:px-8 bg-slate-900/50 overflow-x-auto">
        <button onClick={() => setActiveTab('history')} className={`px-6 py-4 text-sm font-medium transition flex items-center gap-2 relative whitespace-nowrap ${activeTab === 'history' ? 'text-emerald-400' : 'text-slate-300 hover:text-white'}`}><Clock size={16} /> Historial Diario{activeTab === 'history' && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-emerald-400 rounded-t-full shadow-[0_0_8px_rgba(52,211,153,0.5)]" />}</button>
        <button onClick={() => setActiveTab('users')} className={`px-6 py-4 text-sm font-medium transition flex items-center gap-2 relative whitespace-nowrap ${activeTab === 'users' ? 'text-emerald-400' : 'text-slate-300 hover:text-white'}`}><Users size={16} /> Directorio{activeTab === 'users' && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-emerald-400 rounded-t-full shadow-[0_0_8px_rgba(52,211,153,0.5)]" />}</button>
        <button onClick={() => setActiveTab('schedules')} className={`px-6 py-4 text-sm font-medium transition flex items-center gap-2 relative whitespace-nowrap ${activeTab === 'schedules' ? 'text-emerald-400' : 'text-slate-300 hover:text-white'}`}><Calendar size={16} /> Turnos & Horarios{activeTab === 'schedules' && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-emerald-400 rounded-t-full shadow-[0_0_8px_rgba(52,211,153,0.5)]" />}</button>
      </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-800/50">
                    {history.length === 0 ? <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-500"><div className="flex flex-col items-center gap-3"><Clock size={40} className="text-slate-700" /><p>No hay registros</p></div></td></tr> : history.map((rec) => (
                      <tr key={rec.id} className="hover:bg-slate-800/30 transition">
                        ...
                        <td className="px-6 py-4 text-center">
                          <button onClick={() => openJustifyModal(rec)} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-xs font-medium transition inline-flex items-center gap-1.5"><Edit3 size={14} /> Justificar</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-800/50">
                    {filteredUsers.length === 0 ? <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-500">Sin usuarios</td></tr> : filteredUsers.map((u) => {
                      const isEditable = user ? canManage(user, u) : false;
                      return (
                        <tr key={u.id} className="hover:bg-slate-800/30 transition group">
                          <td className="px-6 py-4 font-mono text-slate-300">{u.dni}</td>
                          <td className="px-6 py-4 font-medium text-white">{u.firstName} {u.lastName}<div className="text-xs text-slate-300 font-normal">{u.email}</div></td>
                          <td className="px-6 py-4"><span className="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-slate-800 text-slate-200">{u.role}</span></td>
                          <td className="px-6 py-4">
                            {u.schedule ? (
                              <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider inline-flex items-center gap-1 ${
                                u.schedule.name.toLowerCase().includes('mañana') || u.schedule.name.toLowerCase().includes('manana')
                                  ? 'bg-amber-500/10 text-amber-400'
                                  : u.schedule.name.toLowerCase().includes('tarde')
                                  ? 'bg-sky-500/10 text-sky-400'
                                  : u.schedule.name.toLowerCase().includes('noche')
                              ...
                              <span className="text-xs text-slate-400 italic">Sin turno</span>
                            )}
                          </td>
                          <td className="px-6 py-4">
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider inline-flex items-center gap-1 ${u.isActive ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}><div className={`w-1.5 h-1.5 rounded-full ${u.isActive ? 'bg-emerald-400' : 'bg-rose-400'}`} />{u.isActive ? 'Activo' : 'Inactivo'}</span>
                          </td>
                          <td className="px-6 py-4 text-right">
                            {isEditable ? (
                              <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={async () => { await api.put(`/users/${u.id}`, { isActive: !u.isActive }); fetchUsers(); }} className="p-2 text-slate-200 hover:text-white rounded-lg transition" title="Cambiar Estado">{u.isActive ? <ToggleRight size={18} /> : <ToggleLeft size={18} />}</button>
                                <button onClick={() => openUserModal(u)} className="p-2 text-slate-200 hover:text-emerald-400 rounded-lg transition" title="Editar"><Edit3 size={18} /></button>
                                <button onClick={async () => { if(confirm('¿Eliminar?')){ await api.delete(`/users/${u.id}`); fetchUsers(); } }} className="p-2 text-slate-200 hover:text-rose-400 rounded-lg transition" title="Eliminar"><Trash2 size={18} /></button>
                              </div>
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {schedules.length === 0 ? <tr><td colSpan={7} className="py-12 text-center text-slate-500">Sin turnos</td></tr> : schedules.map((sch) => (
                    <tr key={sch.id} className="hover:bg-slate-800/30 transition group">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => openScheduleModal(sch)} className="p-2 text-slate-200 hover:text-emerald-400 rounded-lg"><Edit3 size={18} /></button>
                          <button onClick={() => handleDeleteSchedule(sch.id)} className="p-2 text-slate-200 hover:text-rose-400 rounded-lg"><Trash2 size={18} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 w-full max-w-xl shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-white">{editingUser ? 'Editar Usuario' : 'Nuevo Usuario'}</h2>
              <button onClick={() => setShowUserModal(false)} className="p-2 text-slate-300 hover:text-white rounded-xl transition"><X size={20} /></button>
            </div>
            <form onSubmit={userForm.handleSubmit(onSubmitUser)} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select {...userForm.register('role')} className={`w-full bg-slate-950 border rounded-xl px-4 py-2 text-white outline-none transition ${userForm.formState.errors.role ? 'border-rose-500' : 'border-slate-800'}`}>{(roleOptions[user?.role || 'HR'] || roleOptions.HR).map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select>
      ...
      {showScheduleModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Clock className="text-emerald-400" size={24} />{editingSchedule ? 'Editar Turno' : 'Nuevo Turno'}</h2>
